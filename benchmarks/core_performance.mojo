...
fn run_comprehensive_benchmarks() raises:
    ...
    for i in range(len(test_sizes)):
        ...
        print(
            "  fib(" + String(n) + "): " + 
            format_duration(avg_time) + " per call" +
            " (benchmark took " + format_duration(benchmark_time) + ")"
        )
    ...
    for i in range(1, len(results)):
        if results[i-1] > 0:
            ...
            print(
                "  fib(" + String(test_sizes[i]) + ") / fib(" + 
                String(test_sizes[i-1]) + ") = " + String(ratio) + "x"
            )
        else:
            print(
                "  fib(" + String(test_sizes[i]) + ") / fib(" + 
                String(test_sizes[i-1]) + ") = N/A (denominator is 0)"
            )
...
fn main() raises:
    ...
    print(
        "  Arithmetic operations: " + format_duration(arithmetic_avg) + 
        " per pair (benchmark took " + format_duration(bench_time) + ")"
    )
